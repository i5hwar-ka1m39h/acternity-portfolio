...
export const BentoGridItem = ({
  ...
  const handleCopy = ()=>{
    navigator.clipboard.writeText('ishwarkalmegh@gmail.com')
    setCopied(true);
  }
      ...
      <div className={`${id===6 && 'flex justify-center'} h-full`}>
          ...
          <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2">
            <div className="flex flex-col gap-3 lg:gap-8">
              {['React.js', 'Next.js', 'Typescript'].map((item, index)=>(
                <span className=" py-2 px-3 lg:py-4 lg:px-3 text-xs lg:text-base rounded-lg opacity-50 lg:opacity-100 text-center bg-[#10132E]" 
                key={index}>{item}</span>
              ))}
            ...
            <div className="flex flex-col gap-3 lg:gap-8">
            <span className=" py-4 px-3 rounded-lg text-center bg-[#10132E]"/>
              {['MongoDB', 'Node.js', 'Express.js'].map((item, index)=>(
                <span className=" py-2 px-3 lg:py-4 lg:px-3 text-xs lg:text-base rounded-lg opacity-50 lg:opacity-100 text-center bg-[#10132E]" 
                key={index}>{item}</span>
              ))}
        ...
        {id === 6 && (
          <div className=" mt-5 relative">
            ...
            <BorderMagicBtn title={copied ? 'Email Copied' : 'Copy My Email'} icon={<IoCopyOutline/>} 
            side="left" otherClasses=' !bg-[#161a31]' handleClick={handleCopy}/>
          </div>
        )}
